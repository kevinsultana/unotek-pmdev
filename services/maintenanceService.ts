...
async function mapServerRequest(item: any, imagesMap: Record<number, string[]>): Promise<MaintenanceRequest> {
  ...
  return {
    ...
    equipment_id: item.equipment?.id || item.equipment_id,
    stage_id: item.stage?.id || item.stage_id,
    maintenance_team_id: item.maintenance_team?.id || item.maintenance_team_id,
